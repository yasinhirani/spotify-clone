  ...
  useEffect(() => {
    if (extractedColors.length > 0) {
      window.ReactNativeWebView?.postMessage(
        JSON.stringify({ id: "BG_COLOR", message: extractedColors[1] })
      );
    }
    return () => {
      window.ReactNativeWebView?.postMessage(
        JSON.stringify({ id: "RESET_BG_COLOR", message: "" })
      );
    };
  }, [extractedColors]);
          ...
          <div className="pt-10 bg-opacity-40">
            <div className="flex flex-col sm:flex-row sm:items-end sm:space-x-10 px-8 md:mt-20">
              ...
              <div className="text-white mt-5 sm:mt-0">
                ...
                <h2 className="font-bold text-4xl">{playlistData.name}</h2>
